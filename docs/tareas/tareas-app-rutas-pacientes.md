...
| Versión | 1.0 |
| Fecha | 2026-08-30 |
| Estado | En ejecución — Fase 4 **iniciada 11/20**. Fase 3 20/20. Fase 2 19/19. Fase 1 código 27/28; único pendiente 1.FE.8 (humanos). 41 pendientes documentadas en §2.1 y [status §6](../info/status.md#6-tareas-pendientes-índice-operativo) |
...
## 2.1 Pendientes a 2026-09-18

Índice operativo de las **41** `[ ]`. Criterios y evidencia siguen en las secciones 4–10.
Gemelo más largo (resto por fase, DoD): [`docs/info/status.md` §6](../info/status.md#6-tareas-pendientes-índice-operativo).

Recuento: Fase 0 16/16; Fase 1 **27/28** (solo 1.FE.8); Fase 2 **19/19**; Fase 3 **20/20**; Fase 4 **11/20**; Fase 5 0/19; Transversal 0/12. Total **93/134**.

**Siguiente corte de producto:** **4.BE.10** (`GET`/`PATCH` notificaciones) o **1.FE.8** (humanos). Fase 3 **20/20**. Fase 4 iniciada. **No** production-ready: ClamAV, DoD PR, usabilidad. **No** arrancar Fase 5 como camino principal. DoD de proceso (PR): status [§3.9](../info/status.md#39-dod-general-no-cumplido-en-el-árbol-de-trabajo-actual) — no es una fila de este WBS.
...
| 4.BE.4 | Job de retención/purga + acta | código | 4.BE.1 y 0.DATA.4 `[x]`. Paralelo, no corte. |
...
| 2.BE.13 | Publicar plan | `[x]` Tras 2.BE.12. HTTP **200** (no 202). Outbox; consumidor **4.BE.9 `[x]`**. |
...
Cadena: 3.BE.1 `[x]` → 3.BE.2 `[x]` → (3.BE.3 `[x]` → 3.BE.4 `[x]` ∥ 3.BE.5 `[x]` ∥ 3.QA.1 `[x]` ; 3.BE.6 `[x]`) → 3.BE.7 `[x]` (ledger 3.BE.14 `[x]`). 3.BE.8 `[x]` tras 3.BE.6. 3.BE.10 `[x]` tras 3.BE.1. 3.BE.12 `[x]` tras 3.BE.4. 3.BE.11 `[x]` y 3.BE.9 `[x]` tras 3.BE.7. 3.FE.1 `[x]` tras 3.BE.7. 3.FE.2 `[x]` tras 3.BE.12. 3.FE.3 `[x]` tras 3.BE.8 y 3.BE.11. 3.BE.13 `[x]` tras 3.BE.11. 3.FE.4 `[x]` tras 3.BE.13. **3.QA.2 `[x]`** cierra la fase.

**Pendientes (0).**

Resto (ids, no criterios): Fase 3 [§7](#7-fase-3--optimización-de-rutas) (0 pendientes); Fase 4 [§8](#8-fase-4--histórico-y-colaboración) (9, 4.BE.10, 4.BE.4 y 4.BE.11 ya en “Ahora”); Fase 5 [§9](#9-fase-5--pruebas-seguridad-y-despliegue) (19, 7 ya en “Ahora”); Transversal [§10](#10-tareas-transversales) (12, 7 ya en “Ahora”). Lista completa de ids en [status §6.3](../info/status.md#63-resto-por-fase-recuento--ids).
...
- [x] 3.BE.9 🔀 **[Paralelizable]** Recalcular ruta al añadir/quitar/modificar paradas de una zona/día (nueva clave de idempotencia por cambio). Ref: RF-20, sección 11.1. Depende de: 3.BE.7. Criterio: modificar una parada invalida la clave anterior y permite nueva optimización. **Hecho 2026-09-11**: `PUT /api/v1/routes/{id}/stops` (`If-Match`, planner/admin) sustituye el conjunto y refresca coordenadas desde la dirección activa; revisión `published` no se muta (abre `draft`). `POST /optimize` incluye `stops_fingerprint` + `osrm_dataset_version` en el ledger (diseño 11.1): misma clave tras el cambio → 409 `IDEMPOTENCY_KEY_REUSE`; clave nueva → 202 y revisión con el nuevo conjunto. Tests: [`test_routing_recalc_e2e.py`](../../backend/tests/test_routing_recalc_e2e.py).
...
- [x] 3.BE.11 🔀 **[Paralelizable]** Endpoint `POST /routes/{id}/publish` (snapshot inmutable de dirección, coordenadas, orden, métricas, geometría y versiones OSRM/OSM). Ref: RF-21, ADR-08, sección 7.6. Depende de: 3.BE.7. Criterio: publicar bloquea edición directa; un cambio posterior crea nueva revisión `draft`. **Hecho 2026-09-11**: `POST /api/v1/routes/{id}/publish` **200** (una transacción; no 202). `Idempotency-Key` (replay; payload distinto → 409 `IDEMPOTENCY_KEY_REUSE`) + `If-Match` sobre `daily_routes.version`. Congela `route_revisions.status=published` + `published_at`; snapshot cifrado AES-GCM en `address_snapshot_ciphertext`; geometría GeoJSON y versiones OSRM/OSM en `constraints_json.snapshot`. Reordenar la publicada → 409 `REVISION_PUBLISHED`. Optimize posterior crea revisión `draft` nueva sin mutar la publicada (`daily_routes.status` → `ready`). Tests: [`test_routing_publish_e2e.py`](../../backend/tests/test_routing_publish_e2e.py). **Caveat**: diseño §8.5 no fija 200/202; se elige 200 como 2.BE.13. Ver [status §3.15](../info/status.md#315-publicar-ruta-200-vs-202).
...
- [x] 3.BE.13 Worker de exportación: PDF, PNG y enlace a app de navegación externa (`POST /routes/{id}/exports`). Ref: RF-22, sección 8.5. Depende de: 3.BE.11. Criterio: los 3 formatos se generan sin incluir datos innecesarios de paciente en el enlace externo. **Hecho 2026-09-11**: `POST /api/v1/routes/{id}/exports` **202**, `Idempotency-Key`, cola `exports` (`worker-exports`). Solo revisión `published`. PDF/PNG esquemáticos (sin teselas OSM) en ObjectStore; `GET /jobs/{id}/artifact`. Enlace Google Maps `/dir/` **solo coords**. Tests: [`test_routing_export_e2e.py`](../../backend/tests/test_routing_export_e2e.py) + [`test_routing_export_render.py`](../../backend/tests/test_routing_export_render.py).
...
- [x] 3.FE.1 🔀 **[Paralelizable]** Optimizador: selección de objetivo, origen/retorno, restricciones, progreso del job. Ref: RF-16, RF-17, RF-19, sección 3.2. Depende de: 3.BE.7. Criterio: E2E lanza optimización y muestra progreso hasta completar. **Hecho 2026-09-11**: UI `/optimizacion` (`OptimizerPage`): plan + ruta diaria, objetivo time/cost, origen, volver al origen o retorno, minutos de atención/jornada, costes si `cost`. `POST /routes/{id}/optimize` con `Idempotency-Key`; polling `GET /jobs/{id}` (diseño 8.5, añadido para el progreso). Vitest: [`OptimizerPage.test.tsx`](../../frontend/src/routing/OptimizerPage.test.tsx) + [`routing.test.ts`](../../frontend/src/api/routing.test.ts). Sin Playwright. Diagnóstico completo = 3.FE.2.
- [x] 3.FE.2 Vista de diagnóstico e inviabilidad con acciones sugeridas. Ref: RF-19, sección 7.5. Depende de: 3.BE.12. Criterio: E2E de caso inviable muestra explicación y acciones disponibles. **Hecho 2026-09-11**: `RouteDiagnostics` en `/optimizacion` si `solver_status=infeasible` o hay `diagnostics`. Títulos `INCOMPATIBLE_WINDOWS` / `ISOLATED_STOP` / `INSUFFICIENT_SHIFT`; acciones `widen window`, `drop stop`, `split route`, `edit order` (no se relajan solas). Vitest: [`RouteDiagnostics.test.tsx`](../../frontend/src/routing/RouteDiagnostics.test.tsx) + caso inviable en [`OptimizerPage.test.tsx`](../../frontend/src/routing/OptimizerPage.test.tsx). Sin Playwright.
- [x] 3.FE.3 Comparación visual base/optimizada (mapa con orden numerado + tabla de métricas). Ref: RF-18, RF-21, sección 3.2. Depende de: 3.BE.8, 3.BE.11. Criterio: E2E muestra ahorro estimado entre ambas rutas. **Hecho 2026-09-11**: `RouteComparison` en `/optimizacion` tras `GET /routes/{id}/comparison` (tabla original/optimizada/ahorro + mapa Leaflet numerado). El comparison incluye `original_stops`/`optimized_stops`. Vitest: [`RouteComparison.test.tsx`](../../frontend/src/routing/RouteComparison.test.tsx). Sin Playwright.
- [x] 3.FE.4 Exportación desde UI (PDF/PNG/enlace de navegación) con confirmación de riesgo si aplica. Ref: RF-22. Depende de: 3.BE.13. Criterio: E2E descarga PDF/PNG y copia enlace de navegación. **Hecho 2026-09-11**: `RouteExport` en `/optimizacion`. PDF/PNG → `POST /exports` + `GET /jobs/{id}/artifact` + descarga. Enlace Google Maps: diálogo de riesgo (proveedor externo, solo coords) y copia al portapapeles. Solo ruta `published`. Vitest: [`RouteExport.test.tsx`](../../frontend/src/routing/RouteExport.test.tsx). Sin Playwright.
...
- [x] 3.QA.2 Suite E2E: recalcular tras modificar paradas, publicar, exportar. Ref: RF-20, RF-21, RF-22, sección 12. Depende de: 3.BE.9, 3.BE.11, 3.BE.13. Criterio: escenario completo en verde en CI. **Hecho 2026-09-11**: [`test_routing_fase3_e2e.py`](../../backend/tests/test_routing_fase3_e2e.py) — PUT paradas (clave optimize anterior 409) → optimize nueva revisión → comparison → publish snapshot → export pdf/png/navigation_link sin PII en el enlace. **1 passed** (Podman/PostGIS). Sin Playwright.
...
- [x] 4.BE.1 🔀 **[Paralelizable]** Consulta de histórico `GET /history/routes?from=&to=&zone=&assignee=&patient_ref=` sobre snapshots (no sobre el paciente actual). Ref: RF-23, RF-24, sección 7.6, 8.5. Depende de: 3.BE.11. Criterio: filtros combinados devuelven snapshots paginados correctos. **Hecho 2026-09-11**: `GET /api/v1/history/routes` (módulo `history`) lista revisiones `published`. Filtros `from`/`to`/`zone`/`assignee`/`patient_ref` (JSONB `snapshot.order.external_ref`, no el `Patient` vivo) + cursor. Campo solo ve sus rutas. Publish guarda `external_ref` en el snapshot. Tests: [`test_history_routes_e2e.py`](../../backend/tests/test_history_routes_e2e.py).
- [x] 4.BE.2 🔀 **[Paralelizable]** Endpoint `PATCH /routes/{id}/stops/{stopId}` idempotente con `If-Match` para reportar ejecución (completada/no completada). Ref: RF-25, sección 7.6, 8.5. Depende de: 3.BE.11. Criterio: conflicto de concurrencia devuelve 409 con estado más reciente. **Hecho 2026-09-18**: `PATCH /api/v1/routes/{id}/stops/{stopId}` body `{status: completed|failed|skipped, completed_at?, failure_reason?}`. `If-Match` = `route_stops.version` (no `daily_routes.version`). Planner/admin o field asignado. Solo ruta `published`/`in_progress`. No muta snapshot ni orden. 409 `STOP_VERSION_CONFLICT` incluye `errors[0]` con el estado más reciente. Primera parada → `in_progress`; todas terminales → `completed`. Tests: [`test_routing_execution_e2e.py`](../../backend/tests/test_routing_execution_e2e.py).
- [x] 4.BE.3 🔀 **[Paralelizable]** Comparativa planificado vs. ejecutado usando `route_metrics.variant = actual`. Ref: RF-25, sección 6.1. Depende de: 4.BE.2. Criterio: endpoint/consulta devuelve desviación entre plan y ejecución real. **Hecho 2026-09-18**: al reportar ejecución se upserta `route_metrics.variant=actual` (viaje = span de `completed_at` de visited − servicio completado; skipped no viaja; **sin GPS**, `distance_m=0`). `GET /routes/{id}/comparison` añade `actual`, `deviation` (actual − optimized; distancia no comparable) y `execution_counts`. Tests: [`test_routing_metrics.py`](../../backend/tests/test_routing_metrics.py) + [`test_routing_actual_e2e.py`](../../backend/tests/test_routing_actual_e2e.py). UI = 4.FE.1.
...
- [x] 4.BE.5 Migración para `share_grants` con CHECK "sujeto XOR token". Ref: sección 6.1. Depende de: 3.BE.1. Criterio: intento de crear grant con sujeto y token a la vez falla por constraint. **Hecho 2026-09-18**: Alembic [`a7c4e91b02d8`](../../backend/alembic/versions/a7c4e91b02d8_sharing_share_grants.py). CHECK `ck_share_grants_subject_xor_token` (sujeto XOR `token_hash`); externo exige `expires_at`; `permission` view/edit; RLS ENABLE+FORCE. Tests: [`test_share_grants_schema.py`](../../backend/tests/test_share_grants_schema.py) + [`test_share_grants_rls.py`](../../backend/tests/test_share_grants_rls.py). Endpoints = 4.BE.6 / 4.BE.7.
- [x] 4.BE.6 Endpoint `POST /routes/{id}/shares` (compartición interna con permiso `view`/`edit`). Ref: RF-27, sección 7.7, 8.6. Depende de: 4.BE.5, 0.BE.5. Criterio: usuario con `view` no puede modificar la ruta; usuario con `edit` sí, si el rol lo permite. **Hecho 2026-09-18**: `POST /api/v1/routes/{id}/shares` 201 (`subject_user_id` + `view`/`edit`, Idempotency-Key, ledger `route.share`). `GET …/shares` lista sin token. Grant activo abre `GET /routes/{id}`; `PATCH` de paradas sigue exigiendo planner/admin (field+edit → 403). Tests: [`test_sharing_internal_e2e.py`](../../backend/tests/test_sharing_internal_e2e.py).
- [x] 4.BE.7 Endpoint `POST /routes/{id}/shares` para token externo (256+ bits, solo hash en BD, expiración obligatoria) y `POST /public-shares/exchange`. Ref: RF-28, sección 7.7, 8.6. Depende de: 4.BE.5. Criterio: token vencido o revocado devuelve 401/410; vista externa oculta campos sensibles por defecto. **Hecho 2026-09-18**: externo sin `subject_user_id` + `expires_at` obligatorio; token `token_urlsafe(32)` una vez; SHA-256 en BD. `POST /public-shares/exchange` 200 vista minimizada (solo `sequence`, sin ref/coords/estado); cookie `sofia_public_share`; `Referrer-Policy: no-referrer`. Lookup `SECURITY DEFINER` (Alembic `b3e8a14c90f1`). 401 `SHARE_TOKEN_INVALID`; 410 `SHARE_EXPIRED` / `SHARE_REVOKED`. Tests: [`test_sharing_external_e2e.py`](../../backend/tests/test_sharing_external_e2e.py).
- [x] 4.BE.8 🔀 **[Paralelizable]** Endpoint `DELETE /shares/{id}` (revocación) con auditoría. Ref: RF-27, RF-28, sección 7.7. Depende de: 4.BE.6, 4.BE.7. Criterio: revocar invalida acceso inmediato (verificado en test de integración). **Hecho 2026-09-18**: `DELETE /api/v1/shares/{id}` 204 (`revoked_at`). Planner/admin. GET interno del beneficiario pasa a 403; exchange externo 410 `SHARE_REVOKED`. Outbox `share.revoked` (sin token/PII; consumidor = 4.BE.9 `[x]`). Cadena `audit_events` = 4.BE.11/12. Tests: [`test_sharing_revoke_e2e.py`](../../backend/tests/test_sharing_revoke_e2e.py).
- [x] 4.BE.9 Outbox transaccional de notificaciones al compartir/reasignar rutas. Ref: RF-29, sección 7.4, 11.1. Depende de: 2.BE.13, 4.BE.6. Criterio: compartir/reasignar genera notificación consumida de forma idempotente (`event_id`). **Hecho 2026-09-18**: no se duplicó `outbox_events`. Alembic [`c4f1d82e90a3`](../../backend/alembic/versions/c4f1d82e90a3_notifications_in_app.py) (`notifications`, UNIQUE `(event_id, recipient_id)`, RLS). Eventos `share.created` / `route.reassigned`; el consumidor también procesa `plan.published` y `share.revoked` (este último sin fila in-app). `consume_outbox_event` marca `processed_at`; reejecutar no duplica. Worker cola `notifications`. GET/PATCH = 4.BE.10. Tests: [`test_notifications_outbox_e2e.py`](../../backend/tests/test_notifications_outbox_e2e.py) + [`test_notifications_schema.py`](../../backend/tests/test_notifications_schema.py).
...
- [x] 4.FE.1 Vista de histórico: búsqueda por fecha/zona/usuario/referencia, comparación plan/ejecución, acceso a snapshots. Ref: RF-24, RF-25, sección 3.2. Depende de: 4.BE.1, 4.BE.3. Criterio: E2E filtra histórico y muestra comparación planificado vs. ejecutado. **Hecho 2026-09-18**: UI `/historico` (`HistoryPage`). Filtros `from`/`to`/zona/visitador/`patient_ref` → `GET /history/routes`. Snapshot de paradas con `external_ref` (no el paciente vivo) + mapa. Comparativa plan vs. ejecución (`GET /comparison`: `actual`/`deviation`/`execution_counts`; distancia no reportada). Vitest: [`HistoryPage.test.tsx`](../../frontend/src/history/HistoryPage.test.tsx) + [`history.test.ts`](../../frontend/src/api/history.test.ts). Sin Playwright.
- [x] 4.FE.2 🔀 **[Paralelizable]** Panel de compartición: usuarios, permiso, caducidad, revocación, vista previa minimizada, confirmación de riesgo externo. Ref: RF-27, RF-28, sección 3.2. Depende de: 4.BE.6, 4.BE.7. Criterio: E2E crea, revisa vista previa y revoca una compartición externa. **Hecho 2026-09-18**: UI `/compartir` (`SharingPage`). Interna (usuario + view/edit) y externa (caducidad + checkbox de riesgo). Token una vez; preview vía `POST /public-shares/exchange` (solo orden). Revocar `DELETE /shares/{id}`. Vitest: [`SharingPage.test.tsx`](../../frontend/src/sharing/SharingPage.test.tsx) + [`sharing.test.ts`](../../frontend/src/api/sharing.test.ts). Sin Playwright.
...
- [x] 4.FE.5 🔀 **[Paralelizable]** Ruta de campo: estado completada/no completada, tolerante a reconexión, sin almacenar teselas públicas offline. Ref: RF-25, sección 3.2, 10.2. Depende de: 4.BE.2. Criterio: E2E marca parada completada tras reconexión simulada sin duplicar el reporte. **Hecho 2026-09-18**: UI `/campo` (`FieldPage`): orden, ventana, mapa en línea (teselas OSM, sin caché offline). Completada / omitir / no completada. Corte de red → cola `sessionStorage` (`sofia_field_outbox`, no teselas). Reconectar: `reportStopExecutionResilient`; 409 `STOP_VERSION_CONFLICT` con el mismo estado = ya aplicado, sin segundo PATCH. `GET /routes/{id}` abierto a field asignado con status/version/coords/`external_ref`. Vitest: [`FieldPage.test.tsx`](../../frontend/src/field/FieldPage.test.tsx) + [`routing.test.ts`](../../frontend/src/api/routing.test.ts). Sin Playwright.
...
- **Wave 3**: cerrada. 3.QA.2 `[x]` 2026-09-11. Fase 3 **20/20**.

### Fase 4 — Histórico y colaboración

- **Wave 1** (parcial): 4.BE.1 `[x]` 2026-09-11. 4.BE.2 `[x]` 2026-09-18.
- **Wave 2**: 4.BE.5–8 `[x]` y 4.FE.2 `[x]` 2026-09-18. **4.BE.9 `[x]`** 2026-09-18 (consumidor outbox). 4.BE.10 desbloqueado.
